...
data class FaceDetectorBuilder(var context: Context) {

    private var cameraView: CameraView? = null
    private var cameraOverlayView: ImageView? = null

    fun from(cameraView: CameraView): FaceDetectorBuilder = apply {
        this.cameraView = cameraView
    }

    fun into(cameraOverlayView: ImageView): FaceDetectorBuilder = apply {
        this.cameraOverlayView = cameraOverlayView

    }

    fun build(): FrameProcessor {

        class FullFacialProcess : FrameProcessor {
            ...
            override fun process(frame: Frame) {
                ...
                faceDetector.detectInImage(firebaseVisionImage)
                    .addOnSuccessListener {
                        cameraOverlayView!!.setImageBitmap(null)
                        ...
                        for (face in it) {
                            ...
                            it.forEach { face ->
                                ...
                                val l_eye = face.leftEyeOpenProbability
                                val r_eye = face.rightEyeOpenProbability

                                val startIndex = face.boundingBox.left + 50
                                dotPaint.textSize = 28F
                                canvas.drawText(
                                    "Smiling : ${smiling * 100}%\n",
                                    (startIndex + 30).toFloat(),
                                    (startIndex - 50).toFloat(),
                                    dotPaint
                                )
                                canvas.drawText(
                                    "Left EYE : ${l_eye * 100}%\n",
                            ...
                            if (cameraView!!.facing == Facing.FRONT) {
                                ...
                                cameraOverlayView!!.setImageBitmap(flippedBitmap)
                            } else {
                                cameraOverlayView!!.setImageBitmap(bitmap)
                            }
                        }
